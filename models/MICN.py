...
class Model(nn.Module):
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        print('dec_out2', dec_out.shape, 'trend2', trend.shape)
        dec_out = dec_out[:, -self.pred_len:, :] + trend[:, -self.pred_len:, :]
        print('dec_out[:, -self.pred_len:, :]', dec_out[:, -self.pred_len:, :].shape, 'trend[:, -self.pred_len:, :]', trend[:, -self.pred_len:, :].shape)
        return dec_out
    ...
    def classification(self, x_enc, x_mark_enc):
        ...
        seasonal_init_enc, trend = self.decomp_multi(x_enc)
        # print('seasonal_init_enc',seasonal_init_enc.shape,'trend',trend.shape)
        # embedding
        dec_out = self.dec_embedding(seasonal_init_enc, None)
        # print('dec_out1', dec_out.shape, 'trend1', trend.shape)
        dec_out = self.conv_trans(dec_out)
        # print('dec_out2', dec_out.shape, 'trend2', trend.shape)
        if dec_out.shape[1] > trend.shape[1]:
            dec_out = dec_out[:, :trend.shape[1], :]
        elif dec_out.shape[1] < trend.shape[1]:
            trend = trend[:, :dec_out.shape[1], :]
        dec_out = dec_out + trend
